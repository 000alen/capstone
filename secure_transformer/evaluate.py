...
class Evaluator:
    ...
    def evaluate_dataset(self, split: str = "test") -> Dict[str, float]:
        ...
        dataloader = DataLoader(
            dataset, batch_size=self.config.batch_size, shuffle=False, num_workers=4
        )
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                if num_batches % 100 == 0:
                    ...
                    logging.info(
                        f"Batch {num_batches}, current perplexity: {current_ppl:.2f}"
                    )
        ...
        with torch.no_grad():
            for _ in range(num_samples):
                ...
                orthogonality_error = (
                    (
                        rotation @ rotation.t()
                        - torch.eye(rotation.size(0), device=rotation.device)
                    )
                    .norm()
                    .item()
                )
...
def main():
    ...
    parser.add_argument(
        "--num_samples", type=int, default=5, help="Number of text samples to generate"
    )

    parser.add_argument(
        "--analyze_security", action="store_true", help="Analyze security properties"
    )
