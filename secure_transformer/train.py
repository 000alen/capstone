...
class WikiTextDataset(Dataset):
    ...
    def __init__(self, split: str, tokenizer, sequence_length: int):
        ...
        logging.info(f"Loaded {split} split: {len(self.tokens)} tokens, {self.num_sequences} sequences")
...
class Trainer:
    ...
    def _load_data(self) -> Tuple[DataLoader, DataLoader]:
        ...
        val_dataset = WikiTextDataset("validation", tokenizer, self.config.sequence_length)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train secure transformer on WikiText-103")
